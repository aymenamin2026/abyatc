...
export default function ProductCard({
  ...
  const name =
    product.name?.[lang] || product.name?.en || product.name || "Product Name";
  ...
  const getDisplayPrice = () => {
    if (product.variations && product.variations.length > 0) {
      return parseFloat(
        product.variations[0].price || product.base_price || "0",
      ).toFixed(2);
    }
    ...
    <>
      ...
      >
        ...
        <div className="relative aspect-[3/4] bg-muted/20 rounded-[24px] sm:rounded-[28px] overflow-hidden mb-4 sm:mb-5 border border-border/40 group-hover:border-primary/30 group-hover:shadow-[0_20px_50px_rgba(var(--primary-rgb),0.08)] transition-all duration-500 z-10">
          ...
          <button
            ...
            title={
              isFav
                ...
                : t("add_to_wishlist" as any, lang as any)
            }
          >
            <Heart
              className={`w-3.5 h-3.5 sm:w-4 sm:h-4 transition-all ${isFav ? "fill-current" : ""} ${isTogglingThis ? "animate-pulse" : ""}`}
            />
          </button>
          ...
          >
            ...
            <button
              ...
              className="w-8 h-8 sm:w-9 sm:h-9 bg-muted/40 text-foreground/80 border border-border/60 rounded-full flex items-center justify-center hover:bg-background hover:text-foreground transition-all duration-300 shadow-sm hover:scale-105 active:scale-95"
            ...
            <a
              ...
              className="w-8 h-8 sm:w-9 sm:h-9 bg-emerald-500 text-white rounded-full flex items-center justify-center hover:bg-emerald-600 transition-all duration-300 shadow-md hover:scale-105 active:scale-95"
              title={
            ...
            >
              ...
              >
                <path
                  fillRule="evenodd"
                  d="M12.21 2c-5.464 0-9.91 4.39-9.91 9.782 0 1.723.454 3.407 1.316 4.898L2 22l5.485-1.422a9.96 9.96 0 004.723 1.185c5.464 0 9.91-4.39 9.91-9.781C22.12 6.39 17.674 2 12.21 2zm5.727 13.914c-.244.68-1.22 1.332-1.83 1.4-.61.07-1.373.13-3.832-.86-2.46-.99-4.04-3.46-4.162-3.62-.122-.17-1.012-1.33-1.012-2.53 0-1.2.634-1.8.854-2.03.22-.24.488-.3.653-.3.164 0 .33.01.47.01.147 0 .348-.06.543.4.195.47.67 1.61.73 1.73.061.12.1.26.02.42-.08.17-.183.28-.317.43-.134.15-.28.34-.4.48-.135.15-.275.31-.116.58.16.27.707 1.15 1.513 1.86.1.09.81.72 1.636 1.05.25.1.445.08.61-.09.214-.22.915-1.05 1.16-1.41.244-.36.488-.3.824-.18.335.12 2.122.99 2.488 1.17.366.18.61.27.695.41.085.15.085.86-.159 1.54z"
                  clipRule="evenodd"
                />
              </svg>
            </a>
        ...
        <div className="flex flex-col flex-1 px-1 sm:px-2">
          {/* CATEGORY METADATA */}
          <div className="text-[9px] sm:text-[10px] uppercase tracking-[0.2em] font-medium text-muted-foreground/80 mb-1.5 sm:mb-2">
