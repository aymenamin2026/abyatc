...
export default function TrackOrderPage() {
  ...
  const performSearch = async (searchQuery: string, isSilent = false) => {
    if (!searchQuery.trim()) return;

    // إذا كان التحديث صامتاً في الخلفية لا نجعل الشاشة تومض بـ loading
    if (!isSilent) {
      setLoading(true);
    }
    ...
    try {
      ...
      setOrder(data.order);
      if (!isSilent) setError(''); // تنظيف الأخطاء إذا كان فحصاً يدوياً ناجحاً
    } catch (err: any) {
      if (!isSilent) {
  ...
  // 3. التتبع التلقائي بمجرد الدخول من خلال الرابط
  useEffect(() => {
  ...
  // 💡 الميزة المضافة: مراقبة صامتة لتحديثات الآدمن كل 10 ثوانٍ دون إزعاج المستخدم
  useEffect(() => {
    // نعتمد على رقم الطلب الحالي الموجود في الـ state كمؤشر لبدء المراقبة
    if (!order?.order_number) return;

    const interval = setInterval(() => {
      performSearch(order.order_number, true); // استدعاء صامت (true) لتحديث الداتا خلف الكواليس
    }, 10000); // 10 ثوانٍ
                      ...
                      <div key={history.id} className="relative flex flex-col md:flex-row items-start md:items-center group">
                        ...
                        <div className="bg-muted border border-border rounded-xl p-4 flex-1 w-full ml-6 md:ml-0 rtl:ml-0 rtl:mr-6 rtl:md:mr-0">
                          ...
                          <h4 className="font-medium text-foreground capitalize mb-1">
                            {lang === 'ar' ? `تغيرت الحالة إلى ${history.status}` : `Status changed to ${history.status}`}
                          </h4>
