  ...
  return (
    <>
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start relative">
        ...
        <div className="w-full lg:hidden flex items-center justify-between p-3 sm:p-4 rounded-2xl border border-border/40 bg-card/60 backdrop-blur-xl mb-4 shadow-sm">
          ...
          <button
            ...
            className="relative flex items-center gap-2 text-xs tracking-widest uppercase font-medium bg-primary text-primary-foreground px-4 py-2.5 sm:px-5 sm:py-3 rounded-xl shadow-md hover:opacity-90 active:scale-95 transition-all"
          >
            <SlidersHorizontal className="w-3.5 h-3.5" />
            {t("filters", lang)}
            {activeFiltersCount > 0 && (
              <span className="absolute -top-1.5 -end-1.5 w-4 h-4 bg-red-500 text-white rounded-full flex items-center justify-center text-[9px] shadow-sm">
                {activeFiltersCount}
              </span>
            )}
          </button>
                  ...
                  return (
                    <button
                      ...
                      ${isSelected
                          ...
                          : "border-border/60 bg-muted/20 hover:border-foreground/40 hover:text-foreground hover:bg-muted"}`}
                    >
                      {sLocal}
                    </button>
                  );
                ...
                {apiColors.map((color: any) => {
                  ...
                  return (
                    <button
                      ...
                      ${bgClass} ${isSelected ? "ring-2 ring-primary ring-offset-2 ring-offset-background scale-110" : "hover:scale-110 hover:shadow-md"}`}
                    >
                      {isSelected && (
        ...
        <AnimatePresence>
          {mobileFiltersOpen && (
            <>
              ...
              {/* Filter Content Body - 2. دعم الـ RTL في الموبايل */}
              <motion.div
                initial={{ x: isRtl ? "100%" : "-100%" }}
                animate={{ x: 0 }}
                exit={{ x: isRtl ? "100%" : "-100%" }}
                transition={{ type: "spring", damping: 25, stiffness: 200 }}
                className="fixed inset-y-0 start-0 w-full max-w-xs sm:max-w-sm bg-card/95 backdrop-blur-3xl border-e border-border/40 p-6 sm:p-8 z-[101] overflow-y-auto lg:hidden shadow-2xl flex flex-col justify-between"
              >
                <div className="space-y-8">
                  <div className="flex items-center justify-between pb-4 border-b border-border/40">
                    <h2 className="text-sm tracking-[0.2em] uppercase font-medium text-foreground">{t("filters", lang)}</h2>
                    <button onClick={() => setMobileFiltersOpen(false)} className="p-1.5 rounded-full hover:bg-muted transition-colors">
                      <X className="w-5 h-5 text-muted-foreground hover:text-foreground" />
                    </button>
                  </div>

                  {/* 3. إضافة خيار الترتيب للموبايل (كان مفقوداً!) */}
                  <div>
                    <h3 className="text-xs tracking-widest font-medium uppercase text-muted-foreground mb-3 flex items-center gap-2">
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full appearance-none bg-muted/30 border border-border/60 rounded-xl px-4 py-3 font-medium text-foreground focus:outline-none focus:border-primary transition-all text-sm"
                      >
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pe-2 custom-scrollbar">
                      <button
                        onClick={() => handleCategoryClick("All")}
                        className={`text-start py-2.5 px-3 text-sm rounded-xl transition-all ${activeCategory === "All" ? "bg-primary text-primary-foreground font-medium shadow-sm" : "text-foreground/80 hover:bg-muted"}`}
                      >
                        {t("all", lang) || "All"}
                      </button>
                      {categories.map((cat) => {
                        ...
                        return (
                          <button
                            key={cat.id}
                            onClick={() => handleCategoryClick(catSlug)}
                            className={`text-start py-2.5 px-3 text-sm rounded-xl transition-all ${isSelected ? "bg-primary text-primary-foreground font-medium shadow-sm" : "text-foreground/80 hover:bg-muted"}`}
                          >
                            {catName}
                          </button>
                        );
                        ...
                        {apiSizes.map((size: any) => {
                          ...
                          return (
                            <button
                              key={size.id}
                              onClick={() => toggleSize(sEn)}
                              className={`rounded-xl text-xs py-3 text-center transition-all border ${selectedSize === sEn ? "border-primary bg-primary text-primary-foreground font-medium shadow-sm" : "border-border/60 bg-muted/20 text-foreground"}`}
                            >
                              {sLocal}
                            </button>
                          );
                ...
                <div className="sticky bottom-0 pt-6 mt-6 bg-card/95 backdrop-blur-3xl border-t border-border/40">
                  <button
                    onClick={() => setMobileFiltersOpen(false)}
                    className="w-full bg-primary text-primary-foreground font-medium py-4 rounded-xl text-center text-sm tracking-wide shadow-lg shadow-primary/20 hover:opacity-90 active:scale-[0.98] transition-all"
                  >
                    {t("view_details", lang) /* أو نص يعبر عن 'عرض النتائج' */} ({sortedProducts.length})
                  </button>
                </div>
              </motion.div>
            </>
        ...
        {/* PRODUCTS GRID AREA */}
        <div className="flex-1 w-full min-w-0"> {/* min-w-0 لمنع تجاوز العناصر العرض */}
            ...
            <div className="flex items-center gap-3 text-xs tracking-widest uppercase text-muted-foreground">
              ...
              <div className="relative">
                <select
                  value={sortOption}
                  onChange={(e) => setSortOption(e.target.value)}
                  className="appearance-none bg-muted/20 border border-border/60 hover:border-foreground/40 rounded-xl ps-4 pe-10 py-2.5 font-medium text-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all cursor-pointer text-xs"
                >
                  <option value="newest" className="bg-card text-foreground">{t("sort_newest", lang)}</option>
                  <option value="featured" className="bg-card text-foreground">{t("sort_featured", lang)}</option>
                  <option value="price_asc" className="bg-card text-foreground">{t("sort_price_asc", lang)}</option>
          ...
          {sortedProducts.length === 0 ? (
            <div className="text-center py-28 p-6 rounded-[32px] border border-dashed border-border/60 bg-muted/10 backdrop-blur-sm">
              <p className="text-sm tracking-widest text-muted-foreground font-light">
                {t("no_products", lang)}
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-y-10 gap-x-6 sm:gap-x-8">
